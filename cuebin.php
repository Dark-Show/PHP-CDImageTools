...
function cli_display_help ($argv) {
	echo ("Cuebin Conversion Tool v" . VERSION . "\n");
	echo ("  Arguments:\n");
	echo ("    -cue \"FILE.CUE\"          Input CUE file\n");
	echo ("    -basename \"DISC_NAME\"    Output filename\n");
	echo ("    -output \"PATH/\"          Output directory\n");
	echo ("    -concatenate             Concatenate CUE/BIN files\n");
	echo ("    -split                   Split CUE/BIN into file per track\n");
	echo ("    -audio flac|wave|raw     Compress audio tracks to format\n\n");
	echo ("  Example Usages:\n");
	echo ("    " . $argv[0] . " -cue \"input.cue\" -output \"output/\" -split\n");
	echo ("    " . $argv[0] . " -cue \"input.cue\" -output \"output/\" -concatenate\n");
	echo ("    " . $argv[0] . " -cue \"input.cue\" -output \"output/\" -audio flac\n");
	die();
}
