	...
	public function load_cue ($cue_file) {
		...
		foreach ($cue as $line) { // Process each line
			if (strtolower (substr ($line, 0, 4)) == "file") { // Process FILE lines, use for all tracks until next FILE line
				...
				if (($p1 = strpos ($line, '"')) === false) // No quotes
					$file = trim (substr (trim ($line), 5, strlen ($line) - (strlen ($type) + 6))); // Store file from between FILE and TYPE
				else {
					$p1++; // Exclude quote
					$p2 = strpos ($line, '"', $p1) - $p1; // Process
					$file = substr ($line, $p1, $p2);  // Store file
				}
			...
			} else if (strtolower (substr (trim ($line), 0, 5)) == "track") { // Process TRACK
				...
				$t = explode (' ', trim ($line));
				$ntrack = (int)$t[1];   // Store track number
				$track['mode'] = $t[2]; // Save Mode
